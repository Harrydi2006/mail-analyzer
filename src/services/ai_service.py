...
class AIService:
    ...
    def __init__(self, config: Config):
        ...
        self.custom_judgement_prompt = self.ai_config.get('custom_judgement_prompt', '')
        self.focus_keywords = self.ai_config.get('focus_keywords', [])
...
- 将相对时间换算为具体日期（例：“明天”={ (current_time + timedelta(days=1)).strftime('%Y-%m-%d') }）。
- 对于持续数月或更长的事项，只需给出开始日期和截至日期，视为时间点/截止类事件（duration_type 设为 deadline），不要展开中间的日程。
...
- 时间必须是具体日期时间，禁止占位符，如 "YYYY-MM-DD HH:MM:SS"。
    ...
    def analyze_email_content(self, content: str, subject: str = '', max_retries: int = 2, user_id: int = None, reference_time: datetime = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_id:
                ...
                custom_judgement_prompt = _pick(user_ai_config.get('custom_judgement_prompt'), self.custom_judgement_prompt)
                focus_keywords = user_ai_config.get('focus_keywords') if user_ai_config.get('focus_keywords') else self.focus_keywords
            else:
                # 使用默认配置
            ...
            for attempt in range(max_retries + 1):
                ...
                original_custom_judgement_prompt = self.custom_judgement_prompt
                original_focus_keywords = self.focus_keywords
                ...
                self.custom_judgement_prompt = custom_judgement_prompt
                ...
                self.custom_judgement_prompt = original_custom_judgement_prompt
                self.focus_keywords = original_focus_keywords
    ...
    def get_provider_info(self) -> Dict[str, Dict[str, Any]]:
        ...
        providers = {
            ...
            'deepseek': {
                'name': 'DeepSeek',
                'description': 'DeepSeek API（OpenAI兼容）',
                'api_key_required': True,
                'base_url_optional': False,
                'default_model': '',
                'pricing': '按量计费',
                'features': ['base_url: https://api.deepseek.com/v1', '申请: https://platform.deepseek.com']
            },
            'siliconflow': {
                'name': '硅基流动',
                'description': 'SiliconFlow API（OpenAI兼容）',
                'api_key_required': True,
                'base_url_optional': False,
                'default_model': '',
                'pricing': '按量计费',
                'features': ['base_url: https://api.siliconflow.cn/v1', '申请: https://cloud.siliconflow.cn/']
            },
            'volcengine': {
                'name': '火山引擎（豆包）',
                'description': '火山引擎豆包兼容模式',
                'api_key_required': True,
                'base_url_optional': False,
                'default_model': '',
                'pricing': '按量计费',
                'features': ['base_url: https://ark.cn-beijing.volces.com/api/v3/chat/completions', '申请: https://console.volcengine.com/']
            },
            'dashscope': {
                'name': '阿里云百炼',
                ...
                'default_model': '',
                'pricing': '按量计费',
                'features': ['base_url: https://dashscope.aliyuncs.com/compatible-mode/v1', '申请: https://bailian.console.aliyun.com']
            },
            'moonshot': {
                'name': '月之暗面',
                'description': 'Moonshot API（OpenAI兼容）',
                'api_key_required': True,
                'base_url_optional': False,
                'default_model': '',
                'pricing': '按量计费',
                'features': ['base_url: https://api.moonshot.cn/v1', '申请: https://platform.moonshot.cn/']
            }
        }
