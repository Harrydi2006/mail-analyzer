...
class EmailService:
    ...
    def save_email_analysis(self, email_data: Dict[str, Any], analysis_result: Dict[str, Any], user_id: int, email_id: int = None) -> int:
        ...
        try:
            ...
            # 双重保障：再标记一次已处理，避免极端情况下仍显示“未处理”
            try:
                self.email_model.mark_email_processed(email_id, user_id)
            except Exception as _e:
                logger.warning(f"重复标记邮件处理状态失败 email_id={email_id}, user_id={user_id}, err={_e}")
